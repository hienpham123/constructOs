...
interface PurchaseRequestComment {
  id: string;
...
interface PurchaseRequestCommentSectionProps {
  purchaseRequestId: string;
}

export default function PurchaseRequestCommentSection({ purchaseRequestId }: PurchaseRequestCommentSectionProps) {
  const { user } = useAuthStore();
  const [comments, setComments] = useState<PurchaseRequestComment[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const newComment = await materialsAPI.createPurchaseRequestComment(formData);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await materialsAPI.deletePurchaseRequestComment(commentToDelete);
  ...
  const handleEditClick = (comment: PurchaseRequestComment) => {
  ...
  const handleEditSave = async (commentId: string) => {
    ...
    try {
      const updated = await materialsAPI.updatePurchaseRequestComment(commentId, editingContent);
  ...
  const isCurrentUser = (comment: PurchaseRequestComment) => {
  ...
  const handleEditFromPopover = (comment: PurchaseRequestComment) => {
